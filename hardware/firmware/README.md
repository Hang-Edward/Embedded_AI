...
- 状态命令：`STATUS?`
- 蓝色按钮事件：`EVENT BUTTON PRESSED`
- 预留命令：`BUZZER:ON/OFF`、`VIB:ON/OFF`、`OLED:TEXT=...`

当前真实可控制的是 `PA5 / D13`，也就是板载绿色 LED `LD2`。外接 LED 也可以通过 `D13` 加限流电阻接入。

蜂鸣器、震动、OLED 命令会先保存固件状态并返回确认；等外设接线完成后，再把这些状态映射到真实 GPIO / I2C 设备。

## Build Outputs

固件构建产物属于本地生成文件，不需要提交到 GitHub。
